...
//! Windows: direct SendInput via platform::win32 — no enigo, no dispatch.

#[path = "platform/mod.rs"]
mod platform;

// ── macOS path (enigo + main-thread dispatch) ───────────────────────────────
...
mod imp {
    ...
    static MODIFIER_NAMES: Lazy<std::collections::HashSet<&'static str>> =
        Lazy::new(|| {
    ...
    fn parse_mouse_button(button: &str) -> Result<Button, String> {
        match button.to_lowercase().as_str() {
            ...
            _ => Err(format!("Invalid mouse button: {}", button)),
        }
    }
    ...
    pub fn key_chord(chord: &str) -> Result<(), String> {
        let parts: Vec<String> =
            chord.split('+').map(|s| s.trim().to_string()).collect();
        if parts.is_empty() {
        ...
        platform::with_enigo(move |enigo| {
            ...
            enigo
                .key(final_enigo_key, Direction::Click)
                .map_err(|e| format!("Error performing key action: {}", e))?;
        ...
        platform::with_enigo(move |enigo| {
            if animated {
                ...
                for i in 1..=steps {
                    ...
                    enigo
                        .move_mouse(ix as i32, iy as i32, Coordinate::Abs)
    ...
    pub fn mouse_location() -> Result<(i32, i32), String> {
        platform::with_enigo(|enigo| Ok(platform::current_mouse_position(enigo)))
    }
...
// ── Windows path (direct SendInput, no enigo) ────────────────────────────────
...
mod imp {
    ...
    pub fn key_action(key_name: &str, action: &str) -> Result<(), String> {
        platform::key_action(key_name, action)
    }

    pub fn key_chord(chord: &str) -> Result<(), String> {
        platform::key_chord(chord)
    }
    ...
    pub fn move_mouse(x: i32, y: i32, animated: bool) -> Result<(), String> {
        platform::move_mouse(x, y, animated)
    }

    pub fn mouse_button(button: &str, action: &str, count: i32) -> Result<(), String> {
        platform::mouse_button(button, action, count)
    }

    pub fn mouse_scroll(amount: i32, direction: &str) -> Result<(), String> {
        platform::mouse_scroll(amount, direction)
    }

    pub fn mouse_location() -> Result<(i32, i32), String> {
        Ok(platform::current_mouse_position())
    }
